    ...
    if (youtubeID) {
        ...
        if (!error) {
            ...
            if (parts) {
                ...
                } else if([parts objectForKey:@"reason"] && [[parts objectForKey:@"reason"] isKindOfClass:[NSArray class]] > 0)  {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"Playback Failed" message:[[parts objectForKey:@"reason"] objectAtIndex:0] delegate:nil cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
                        [alertView show];
                    });
                }
            }
